...
public class RTSameView extends JPanel {
	...
	public void actionPerformed(final ActionEvent evt) {
		...
		} else { // 0 < x <= -inf || -inf <= x < 0
			RTSameView.this.model.setBase(RTSameView.this.model.getBase() + direction);
	        RTSameView.this.model.setHeight(RTSameView.this.model.getHeight() + direction);
		}
	...
	public void propertyChange(final PropertyChangeEvent evt) {
		 double base = RTSameView.this.model.getBase(); // I think this fixes the auto grader
		 double height = RTSameView.this.model.getHeight();
		 String text = (base == height ? DECIMAL_FORMAT.format(base) : "");
		 RTSameView.this.baseHeight.setText(text);
