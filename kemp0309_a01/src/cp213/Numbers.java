...
public class Numbers {
    ...
    public static double closest(final double target, final double v1, final double v2) {
    	// use absolute value bc example uses negative integers
	return Math.abs(v1 - target) <= Math.abs(v2 - target) ? v1 : v2;
    }
    ...
    public static boolean isPrime(final int n) {
    	...
    	if(n < 2) {
    	 ...
    	 // checkin for odds
    	 for (int i = 2; i <= n / 2; i++) {
             if((n % i) == 0) { // divisible by i means that it cannot be prime
                 return false;
             }
         }
	...
	for(int i = 1; i <= n; i++) {
		sum += 1.0/i; // floating point or sets to integer
	}
